...
export default function ServiceDetail() {
  ...
  return (
    <>
      ...
      <PageBanner
        title={service.title}
        image={hero.image || service.image}
        parent={parent}
      />
        ...
        <div className="container">
          <div className="row">
            <div className="col-lg-8 ftco-animate">
              ...
              {(help.heading || help.body || help.cta?.label) && (
                <div className="pcn-help-card p-4 p-md-5 my-5">
                  {help.heading && <h2 className="mb-3">{help.heading}</h2>}
                  {help.body && <p className="mb-4">{help.body}</p>}
                  {help.cta?.label && (
                    <p className="mb-0">
                      <SmartLink
                        href={help.cta.href}
                ...
                <div className="mt-5 pt-4">
                  <div className="col-md-12 px-0">
                    <h2 className="mb-4 font-weight-bold">
                      {advisors.heading || "Our Legal Advisors"}
                    </h2>
                  </div>
                  <div className="row">
                    {service.advisors.map((l) => (
                      <div className="col-lg-6 col-md-6 mb-4" key={l.slug}>
                        <FlipCard lawyer={l} compact />
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {service.cases?.length > 0 && (
                <div className="mt-5">
                  <div className="col-md-12 px-0">
                    <h2 className="mb-4 font-weight-bold">
                      {section(page, "relatedCases").heading ||
                        "Related Matters in the Record"}
                    </h2>
                  </div>
                  <div className="pcn-case-grid">
                    {service.cases.map((c) => (
                      <CaseCard item={c} key={c.slug} />
                    ))}
                  </div>
                </div>
              )}
