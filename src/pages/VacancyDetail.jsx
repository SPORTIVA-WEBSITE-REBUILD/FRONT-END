...
import { useLayout, usePage, useVacancy, section } from '../hooks/useContent.js';
...
export default function VacancyDetail() {
  ...
  useEffect(() => {
    if (redirectTo && redirectTo !== slug) navigate(`/careers/${redirectTo}`, { replace: true });
  ...
  const applyHref = vacancy.applyUrl
    || (inbox ? `mailto:${inbox}?subject=${encodeURIComponent(vacancy.title)}` : null);
    ...
    <>
      <Seo
        ...
        jsonLd={graph(
          ...
          breadcrumbs([{ label: 'Home', href: '/' }, { label: parent.label, href: parent.href }, { label: vacancy.title }]),
        )}
      />
          ...
          <div className="row">
            <div className="col-lg-8 ftco-animate">
              {vacancy.isClosed && closed.body && (
                <div className="pcn-form-alert pcn-form-alert--error" role="status">{closed.body}</div>
              )}
              ...
              {vacancy.responsibilities?.length > 0 && (
                <>
                  <h2 className="mb-3 mt-5">{d.responsibilities}</h2>
                  <ul>{vacancy.responsibilities.map((r) => <li key={r}>{r}</li>)}</ul>
                </>
              )}
              {vacancy.requirements?.length > 0 && (
                <>
                  <h2 className="mb-3 mt-5">{d.requirements}</h2>
            ...
            <div className="col-lg-4 sidebar pl-lg-5 ftco-animate">
              <div className="sidebar-box ftco-animate">
                ...
                <ul className="list-unstyled">
                  {vacancy.location && <li className="mb-2"><strong>{d.location}</strong> {vacancy.location}</li>}
                  <li className="mb-2"><strong>{d.type}</strong> {term(terms, '', vacancy.employmentType)}</li>
                  {vacancy.workplaceType && <li className="mb-2"><strong>{d.arrangement}</strong> {term(terms, '', vacancy.workplaceType)}</li>}
                  {vacancy.department && <li className="mb-2"><strong>{d.team}</strong> {vacancy.department}</li>}
                  {vacancy.salaryRange && <li className="mb-2"><strong>{d.salary}</strong> {vacancy.salaryRange}</li>}
                  {vacancy.closingDate && (
                    <li className="mb-2"><strong>{d.closes}</strong> <time dateTime={vacancy.closingDate}>{longDate(vacancy.closingDate)}</time></li>
                  )}
                </ul>
              </div>
              ...
              <div className="sidebar-box ftco-animate">
                ...
                <p><Link to="/careers" className="btn btn-primary">{d.viewAll}</Link></p>
              </div>
            </div>
          </div>
        </div>
